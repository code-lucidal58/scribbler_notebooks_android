...
public class RecyclerCustomAdapter extends RecyclerView.Adapter<RecyclerCustomAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position) {
        ...
        imageLoader.DisplayImage(dealsList.get(position).get(Constants.TAG_IMAGE_URL),R.drawable.ic_launcher, viewHolder.imgViewIcon);

        if(dealsList.get(position).get(Constants.TAG_NEW).isEmpty())
            viewHolder.txtViewNewHot.setVisibility(View.GONE);
    }
